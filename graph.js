...
TODO:
  Make Deleting vertices not change the displayed vertices
...
Edge.prototype.nearestLinePoint = function (x, y) {
  ...
  var lineY = lerp(this.v2.y, this.v1.y, t)
...
Vertex.prototype.contains = function (mx, my) {
  return (Math.sqrt(Math.pow(Math.abs(this.x - mx), 2) + Math.pow(Math.abs(this.y - my), 2)) <= 10)
}
...
Intersection.prototype.drawLayering = function (ctx) {
  ...
  // make unit vector
  var xVec = (edge.v1.x - edge.v2.x) * 0.1
  var yVec = (edge.v1.y - edge.v2.y) * 0.1
  // var mag = Math.sqrt(xVec * xVec + yVec * yVec)

  ctx.strokeStyle = 'white'
  ctx.lineWidth = 10
  ctx.beginPath()
  ctx.moveTo(this.x - xVec, this.y - yVec)
  ctx.lineTo(this.x + xVec, this.y + yVec)
  ctx.stroke()
...
function lineIntersect (edge1, edge2) {
  ...
  if ((x === x1 || x === x2 || x === x3 || x === x4) &&
           (y === y1 || y === y2 || y === y3 || y === y4)) { return false }
    // change to an object
  return [true, x, y]
}
    ...
    if (this.vertexNumber) { // this boolean is toggled when n is pressed
      for (var i = 0; i < l; i++) {
        if (vertices[i].size === 10) {
          vertices[i].draw(ctx, vertices[i].num) //- breakCounter)
        } else {
